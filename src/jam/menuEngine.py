...
class ButtonIfClicked:
    def __init__(self):
        self.riddles = []  # Inicjalizacja atrybutu riddles
        self.used = []     # Inicjalizacja atrybutu used
    def Easy(self):
        """Funkcja wybierająca zagadki łatwe."""
        try:
            with open('src/jam/args.json', 'r', encoding='utf-8') as file:
                data = json.load(file)
    ...
    def Hard(self):
        ...
        try:
            with open('src/jam/args.json', 'r', encoding='utf-8') as file:  # Upewnij się, że ścieżka jest poprawna
                data = json.load(file)
